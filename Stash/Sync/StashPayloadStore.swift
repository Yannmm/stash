...
struct StashPayloadStore {
    ...
    func serialize(metadata: SyncMetadata) throws -> Data {
    ...
    func deserializeMetadata(from data: Data) throws -> SyncMetadata {
    ...
    func writeMetadata(_ metadata: SyncMetadata, to url: URL) throws {
        try ensureParentDirectoryExists(for: url)
        try serialize(metadata: metadata).write(to: url, options: .atomic)
    }

    func normalizedMetadata(at url: URL, payloadData: Data) throws -> SyncMetadata? {
